...
load_data_config = {
    ...
    'test_path': 'data/sim_vision_reach_test/',
    'push_path': 'data/sim_push',  # to be modified
    'shuffle': True,
    'state_vec_len': 10,
    'action_vec_len': 2,
...
def init_load_data_config(config):
    load_data_config['size'] = config['train_samples'] + config[
        'validate_samples']
...
def load_states_and_actions():
    ...
    files = natsorted([os.path.join(gif_dir, f) for f in os.listdir(gif_dir) if
                       f.endswith('pkl')])
...
def generate_test_demos():
    gif_dir = load_data_config['test_path']
    gif_prefix = load_data_constants['gif_dir_prefix']
    frames_in_gif = load_data_constants['frames_in_gif']
    print("Generate test demos...")

    files = natsorted([os.path.join(gif_dir, f) for f in os.listdir(gif_dir) if
                       f.endswith('pkl')])
    num_folders = len(files)

    demos = [unpickle(file) for file in files]
    policy_demo_idx = [np.random.choice(n_demo, replace=False, size=1) for
                       n_demo in
                       [demos[i]['demoX'].shape[0] for i in range(num_folders)]]
    ...
    for i in range(num_folders):
        ...
        for idx in selected_cond:
            # Os.append(gif_dir + gif_prefix + '%d/cond%d.samp0.gif' % (
            #     i, idx))
            O = gif_dir + gif_prefix + '%d/cond%d.samp0.gif' % (i, idx)
            Os.append(O)
            Xs.append(demos[i]['demoX'][
                          np.arange(demos[i]['demoX'].shape[0]) ==
                          policy_demo_idx[
                              i]].squeeze())
            Us.append(demos[i]['demoU'][
                          np.arange(demos[i]['demoU'].shape[0]) ==
                          policy_demo_idx[
                              i]].squeeze())
            ...
            selected_demo = dict(selected_demoX=selected_demoX,
                                 selected_demoU=selected_demoU,
                                 selected_demoO=selected_demoO)
            load_data_variables['demos'] = demos
            load_data_variables['test_select_demos'] = selected_demo
...
def load_gif_dirs():
    ...
    train_gif_dirs = [os.path.join(gif_dir, prefix + str(i)) for i in
                      train_index]
    ...
    for i in range(iterations):
        ...
        for folder in folders:
            ...
            gif_choices = np.random.choice(range(len(gifs)), shots,
                                           replace=False)
            gif_name_choices = [os.path.join(folder, gifs[item]) for item in
                                gif_choices]
...
def normalize_states(data):
    ...
    state_vectors = np.vstack(
        (data[i]['demoX'] for i in range(load_data_config['size'])))
    ...
    with open('data/scale_and_bias_%s.pkl' % load_data_config['task_type'],
              'wb') as f:
        pickle.dump({'scale': scale, 'bias': avg}, f)
    ...
    for i in range(load_data_config['size']):
        ...
        data[i]['demoX'] = np.reshape(data[i]['demoX'], (
            -1, load_data_constants['frames_in_gif'], l))
